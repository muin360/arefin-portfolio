...
interface HeroSignatureProps {
  ...
  labStack?: string[];
  labLink?: string;
  variant?: "full" | "portrait-only" | "readouts-only";
}
...
export default function HeroSignature({
  ...
  const showPortrait = variant === "full" || variant === "portrait-only";
  const showReadouts = variant === "full" || variant === "readouts-only";

  return (
    <div className="relative w-full max-w-[420px] mx-auto flex flex-col items-center select-none space-y-5">
