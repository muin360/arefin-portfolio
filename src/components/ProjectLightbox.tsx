      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 ${className}`}>
        {images.map((img, i) => (
          <div
            ...
            className="group relative h-60 sm:h-72 rounded-2xl overflow-hidden border border-white/[0.08] bg-[#0c0f18] hover:border-violet-500/40 cursor-pointer transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-violet-400"
          >
            <Image
              src={img.src}
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex flex-col justify-between p-4 pointer-events-none">
          ...
          >
            ...
            {images.length > 1 && (
              <>
                <button
                  type="button"
                  onClick={handlePrev}
                  className="absolute left-4 sm:left-8 p-3 rounded-2xl bg-[#121622]/80 hover:bg-[#181e2e] text-white border border-white/10 transition-all duration-150 z-10 hidden sm:flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-violet-400"
                ...
                <button
                  type="button"
                  onClick={handleNext}
                  className="absolute right-4 sm:right-8 p-3 rounded-2xl bg-[#121622]/80 hover:bg-[#181e2e] text-white border border-white/10 transition-all duration-150 z-10 hidden sm:flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-violet-400"
            ...
            >
              <Image
                ...
                alt={images[selectedIndex].alt || `Project showcase detail ${selectedIndex + 1}`}
                fill
                priority
            ...
            {images[selectedIndex].caption && (
              <div className="mt-4 px-4 py-2 rounded-xl bg-[#121622]/90 border border-white/10 text-xs sm:text-sm font-mono text-white/80 max-w-2xl text-center">
                {images[selectedIndex].caption}
              </div>
            )}
          </motion.div>
