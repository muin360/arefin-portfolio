...
import { trackWhatsAppClick } from "@/lib/track-event";
...
export default function WhatsAppFab({
  ...
  return (
    <a
      ...
      onClick={() => trackWhatsAppClick("Floating Action Button")}
      aria-label="Chat directly with Arefin Mueen on WhatsApp"
      className={`fixed z-[55] bottom-16 md:bottom-8 right-4 md:right-8 transition-all duration-300 ${
        visible
    ...
    >
      <span className="relative flex items-center gap-2.5 rounded-full bg-[#25D366] hover:bg-[#1ebe57] text-white pl-3.5 pr-4 py-3 shadow-[0_10px_30px_-6px_rgba(37,211,102,0.6)] hover:shadow-[0_14px_36px_-6px_rgba(37,211,102,0.8)] transition-all hover:scale-105 active:scale-95 group">
        {/* Pulsing Aura */}
        <span
          aria-hidden="true"
          className="absolute -inset-1 rounded-full bg-[#25D366]/40 blur-md opacity-70 group-hover:opacity-100 animate-pulse"
        />
        ...
        <span className="hidden sm:inline text-xs font-mono font-semibold tracking-wide pr-0.5">
          WhatsApp Direct
        </span>
