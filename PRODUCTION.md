...
| Name                                  | Required | Notes                                                                                  |
| ------------------------------------- | -------- | -------------------------------------------------------------------------------------- |
| `NEXT_PUBLIC_SITE_URL`                | yes      | e.g. `https://tensorix.ai`. No trailing slash.                              |
...
| `CONTACT_FROM_EMAIL`                  | optional | Default `Tensorix <onboarding@resend.dev>`. After you verify a custom domain in Resend, set this to e.g. `Tensorix <hi@tensorix.ai>`. |
...
2. **URL:** `https://tensorix.ai/api/revalidate`
...
4. After verifying a domain, set `CONTACT_FROM_EMAIL=Tensorix <hi@yourdomain.com>` on Vercel.
...
1. **Search Console** → submit `https://tensorix.ai/sitemap.xml`.
